...
# Supabase client (reads from Streamlit Secrets in the cloud)
@st.cache_resource
def get_client():
    url = st.secrets["supabase"]["url"]
    key = st.secrets["supabase"]["anon_key"]  # use ANON key
    return create_client(url, key)

supabase = get_client()

# Fetch data from Supabase
@st.cache_data(ttl=10)  # auto-refresh every ~10s
def fetch_data(limit=1000):
    ...
    df["DateTime"] = df["created_at"].dt.tz_convert(None)  # strip tz for charts
    return df
